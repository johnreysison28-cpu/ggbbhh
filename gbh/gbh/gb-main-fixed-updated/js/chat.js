...
function _renderMessages(msgs, uid) {
  ...
  msgs.forEach((m, i) => {
    ...
    const receipt   = isLastSent
      ...
      : '';
    html += '<div class="msg-row ' + (isMine ? 'mine' : 'theirs') + '"><div class="msg-bubble-wrap"><div class="msg-bubble">' + escHtml(m.text) + '</div><div class="msg-time">' + timeStr + '</div>' + receipt + '</div></div>';
  });
...
async function sendMessage() {
  ...
  // Save text then clear the input
  const saved   = text;
  inputEl.value = '';
  ...
  try {
    ...
    await chatRef.set({
      ...
      lastMessage: saved.length > 60 ? saved.slice(0, 57) + '…' : saved,
      updatedAt:   now,
    ...
    // Restore typed text so the user doesn't lose it
